...
public class PacketWizard {
    ...
    public static void playBlockBreakAnimation(Player breaker, int animStage, BlockPosition pos) {
        PacketPlayOutBlockBreakAnimation packet = new PacketPlayOutBlockBreakAnimation(breaker.getEntityId() + 1, pos, animStage);

        for (Entity player : breaker.getWorld().getNearbyEntities(breaker.getLocation(), 32, 32, 32)) {
            if (player instanceof CraftPlayer) {
                ((CraftPlayer) player).getHandle().playerConnection.sendPacket(packet);
            }
        }
